...
export default function Signup() {
  ...
  const [role, setRole] = useState("rider"); // <-- new state for role
  ...
  const handleSignUp = async () => {
    if (!fullName) {
     addToast("Please enter your full name!", { type: 'error' });
      return;
    }
...
// Insert user into public.users table
      ...
      if (insertError) {
        ...
        addToast("Error creating user. Please try again.", { type: 'error' });
        setLoading(false);
        return;
      }

     addToast("Check your email to confirm your signup!", { type: 'success' });
  ...
  const handleGoogleSignup = async () => {
    ...
    // Wait for redirect to complete and then check user session
    ...
    <div className="min-h-screen flex items-center justify-center relative bg-gradient-to-tr from-yellow-100 via-yellow-300 to-yellow-500 overflow-hidden px-4">
      ...
      <div className="relative z-10 w-full max-w-lg mx-4 bg-black/70 backdrop-blur-2xl rounded-2xl shadow-2xl px-8 py-10 flex flex-col items-center">
        ...
        <div className="space-y-5 w-full max-w-md">
          ...
          <div className="flex items-center bg-white/10 rounded-md px-3 py-2 shadow border border-gray-600 focus-within:border-yellow-400 transition">
            <span className="text-yellow-400 opacity-80">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M5.121 17.804A7 7 0 1116.95 6.95 7 7 0 015.121 17.804z" />
          ...
          <div className="flex gap-4 mt-2 text-white font-medium">
        ...
        <div className="mt-2 text-gray-300 text-xs text-center">
          ...
          <Link to="/login" className="ml-1 underline hover:text-yellow-400 transition">
            Login →
          </Link>
        </div>
      </div>
